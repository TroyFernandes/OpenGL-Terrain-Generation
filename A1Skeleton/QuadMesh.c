...
#include <string.h>
#include <math.h>
#include <windows.h> //remove this after
#include "QuadMesh.h"
...
bool CreateMemoryQM(QuadMesh* qm)
{
	...
	qm->holes = malloc(sizeof(Holes) * 50);
...
void DrawMeshQM(QuadMesh* qm, int meshSize)
{
	...
	//glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);//Turn into wireframe
	for (int j = 0; j < meshSize; j++)
	{
		for (int k = 0; k < meshSize; k++)
		{
			...
			glEnd();
			//printf("%s%.2f%s%.2f%s%.2f%s", "(", qm->quads[currentQuad].vertices[0]->position.x, ", ", qm->quads[currentQuad].vertices[0]->position.y, ", ", qm->quads[currentQuad].vertices[0]->position.z, ")");
			//printf("%s%.2f%s%.2f%s%.2f%s", "(", qm->quads[currentQuad].vertices[1]->position.x, ", ", qm->quads[currentQuad].vertices[0]->position.y, ", ", qm->quads[currentQuad].vertices[0]->position.z, ")");
			//printf("%s%.2f%s%.2f%s%.2f%s", "(", qm->quads[currentQuad].vertices[2]->position.x, ", ", qm->quads[currentQuad].vertices[0]->position.y, ", ", qm->quads[currentQuad].vertices[0]->position.z, ")");
			//printf("%s%.2f%s%.2f%s%.2f%s", "(", qm->quads[currentQuad].vertices[3]->position.x, ", ", qm->quads[currentQuad].vertices[0]->position.y, ", ", qm->quads[currentQuad].vertices[0]->position.z, ")");
...
void UpdateMesh(QuadMesh * qm) {
	//printf("%d", holeCounter);
	for (int i = 0; i < qm->numVertices; i++) {
		...
		for (int j = 0; j < holeCounter; j++) {
			//One-liner function
			y += -1 * exp(-6 * ((pow(qm->holes[j].x - qm->vertices[i].position.x, 2) + pow(qm->holes[j].z - qm->vertices[i].position.z, 2))));

			
			/*
			Below is the funcion broken up to understand it more easier, and see which value does what.
			To enable the usage of it, uncomment the float variable declarations "a", "b", and "r". 
			Also comment out the one-liner.
			*/

			/*
			r = distance from mesh vertex to a hole
			r = ((Hole X location) - (Mesh Vertex X position))² + ((Hole Z location) - (Mesh Vertex Z position))²
			You need to calculate both the X distance and Z distance otherwise you will create a hole along the whole axis
			*/
			//r = pow(qm->holes[i].x - qm->vertices[j].position.x, 2) + pow(qm->holes[i].z - qm->vertices[j].position.z, 2);

			/*
			b = height
			Negative values will create a hole instead of a hill.
			*/
			//b = -1; 

			/*
			a = width of the hole
			Keep the value negative, which is why there's a minus sign.
			*/
			//a = -6;

			/*
			Sum up the values for each vertex and hole.
			Not summing up the values will remove the previous holes.
			*/
			//y += b * exp(a * r);


		}
		//Apply the Y value to the quad mesh vertex
		qm->vertices[i].position.y = y;
	}
	ComputeNormalsQM(qm);
...
void MakeHole(QuadMesh* qm, float xPos, float zPos) {
	//printf("%s%.2f\n","Making hole at X: ", xPos);
	//printf("%s%.2f\n", "Making hole at Z: ", zPos);
	qm->holes[holeCounter].x = xPos;
	qm->holes[holeCounter].z = zPos;
	holeCounter++;
	UpdateMesh(qm);
}